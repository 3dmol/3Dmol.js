...
import { clamp, Matrix4, Ray } from "../math";
...
/**
 * @param {{ matrixWorld: { getMaxScaleOnAxis: () => number; }; }} group
 * @param {{ intersectionShape: any; boundingSphere: unknown; }} clickable
 * @param {Raycaster} raycaster
 * @param {any[]} intersects
 */
function intersectObject(group, clickable, raycaster, intersects) {
  ...
  for (i = 0, il = intersectionShape.cylinder.length; i < il; i++) {
    if (intersectionShape.cylinder[i] instanceof Cylinder) {
      ...
      normProj = clamp(raycaster.ray.direction.dot(cylinder.direction), -1, 1); // Beta
...
export class Raycaster {
  ...
  setFromCamera(coords, camera) {
    if (!camera.ortho) {
      ...
      this.ray.direction.applyProjection(this._viewProjectionMatrix);
  ...
  intersectObjects(group, objs) {
    ...
    for (var i = 0, l = objs.length; i < l; i++)
      intersectObject(group, objs[i], this, intersects);
