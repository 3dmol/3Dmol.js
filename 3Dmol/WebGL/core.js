...
import {Matrix4, Quaternion, Ray, Vector3} from './math';
...
export class Object3D extends EventDispatcher {
  ...
  geometry;
  /** @type {import("./materials").Material|undefined} */
  material;

  /** @type {number|undefined} */ x;
  /** @type {number|undefined} */ y;
  /** @type {number|undefined} */ z;
  ...
  lookAt(vector) {
    ...
    if (this.rotationAutoUpdate) {
      if (this.useQuaternion === true) this.quaternion.copy(this.matrix.decompose()[1]);
      else this.rotation.setEulerFromRotationMatrix(this.matrix, this.eulerOrder);
    }
  }
...
export class Scene extends Object3D {
  ...
  __addObject(object) {
    ...
    for (let i = 0; i < object.children.length; i++) this.__addObject(object.children[i]);
  }
  ...
  __removeObject(object) {
    ...
    for (let i = 0; i < object.children.length; i++) this.__removeObject(object.children[i]);
  }
}

// add child object requires Scene to be declared
Object3D.prototype.add = function add(object) {
  ...
  if (scene !== undefined && scene instanceof Scene) scene.__addObject(object);
};
...
Object3D.prototype.remove = function remove(object) {
  ...
  if (index !== -1) {
    ...
    if (scene !== undefined && scene instanceof Scene) scene.__removeObject(object);
  }
};
...
export class geometryGroup {
  ...
  setLineIndices() {
    ...
    this.lineArray = new Uint16Array(this.faceidx * 2);
    this.lineidx = this.faceidx * 2;

    for (let i = 0; i < this.faceidx / 3; ++i) {
      const faceoffset = i * 3;
      const lineoffset = faceoffset * 2;
      const a = this.faceArray[faceoffset];
      const b = this.faceArray[faceoffset + 1];
      const c = this.faceArray[faceoffset + 2];

      this.lineArray[lineoffset] = a;
      this.lineArray[lineoffset + 1] = b;
      this.lineArray[lineoffset + 2] = a;
      this.lineArray[lineoffset + 3] = c;
      this.lineArray[lineoffset + 4] = b;
      this.lineArray[lineoffset + 5] = c;
    }
  }
...
export class Geometry extends EventDispatcher {
  ...
  dispose() {
    /** @type {any} */ (this).dispatchEvent({type: 'dispose'});
  }

  get verticies() {
    let vertices = 0;
    for (let g = 0; g < this.groups; g++) vertices += this.geometryGroups[g].vertices;
    return vertices;
  }
}
...
const w0 = new Vector3(); // for cylinders, cylinder.c1 - ray.origin
const v1 = new Vector3(); // all purpose local vector
...
function intersectObject(group, clickable, raycaster, intersects) {
  ...
  if (clickable.boundingSphere !== undefined && clickable.boundingSphere instanceof Sphere) {
  ...
  for (i = 0, il = intersectionShape.cylinder.length; i < il; i++) {
    if (intersectionShape.cylinder[i] instanceof Cylinder) {
      ...
      if (closestDistSq <= radiusSq) {
        ...
        else t = distance = (rayProj - normProj * cylProj - Math.sqrt(discriminant)) / denom;
        ...
        else intersects.push({clickable, distance});
      }
    }
  }
  // lines
  for (i = 0, il = intersectionShape.line.length; i < il; i += 2) {
    ...
    if (closestDistSq < precisionSq && sC * sC < bondLengthSq)
      intersects.push({clickable, distance: tC});
  }
  for (i = 0, il = intersectionShape.sphere.length; i < il; i++) {
    ...
    if (intersectionShape.sphere[i] instanceof Sphere) {
      ...
      if (raycaster.ray.isIntersectionSphere(sphere)) {
        ...
        discriminant =
          distanceToCenter * distanceToCenter - (v1.lengthSq() - sphere.radius * sphere.radius);
