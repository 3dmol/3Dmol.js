...
const d = new Vector3();
function getRotationMatrix(dir) {
...
const basisVectors = (function basisVectorConstructor() {
  const nvecs = [];
...
const cylVertexCache = {
  ...
  getVerticesForRadius(radius, cap, capType) {
    if (typeof this.chache !== 'undefined' && this.cache[radius] !== undefined)
      if (this.cache[radius][cap + capType] !== undefined) return this.cache[radius][cap + capType];
    ...
    const widthSegments = w; // 16 or however many
    // basis vectors for
    // cylinder
      ...
      for (x = 0; x <= widthSegments; x++) {
        ...
        if (!polar || x === 0) {
          if (x < widthSegments) {
            ...
            if (cap === 1) vertex.y = 0;
            else vertex.y = radius * Math.cos(thetaStart + v * thetaLength);
...
const sphereVertexCache = {
  ...
  getVerticesForRadius(radius) {
    if (typeof this.cache[radius] !== 'undefined') return this.cache[radius];

    const obj = {
      /** @type {import('./specs').Vector3Like[]} */
      vertices: [],
      /** @type {number[][]} */
      verticesRows: [],
      /** @type {import('./specs').Vector3Like[]} */
    ...
    for (y = 0; y <= heightSegments; y++) {
      ...
      for (x = 0; x <= widthSegments; x++) {
        ...
        const n = new Vector3(vertex.x, vertex.y, vertex.z);
        n.normalize();
...
export default class GLDraw {
   ...
   * @param {import('./specs').Vector3Like | undefined} from
   * @param {import('./specs').Vector3Like | undefined} to
   * @param {number} radius
   * @param {import('./WebGL/core/Color').default} color
   * @param {import('./enum/CAP').default} fromCap - 0 for none, 1 for flat, 2 for round
   * @param {import('./enum/CAP').default} toCap = 0 for none, 1 for flat, 2 for round
   *
   * */
  static drawCylinder(geo, from, to, radius, color, fromCap, toCap) {
    ...
    let {vertices} = vobj;
    let {normals} = vobj;
    let {verticesRows} = vobj;
    const toRow = verticesRows[h / 2];
    const fromRow = verticesRows[h / 2 + 1];
  ...
  /** Create a cone
   * @param {import('./WebGL/core').Geometry} geo
   * @param {import('./specs').AtomSpec} from
   * @param {import('./specs').AtomSpec} to
   * @param {number} radius
   * @param {import('./WebGL/core/Color').default} color
   */
  static drawCone(geo, from, to, radius, color) {
    ...
    const dir = {x: to.x, y: to.y, z: to.z};
    dir.x -= from.x;
    dir.y -= from.y;
    dir.z -= from.z;
    ...
    offset = start * 3;
    const ndir = new Vector3(dir.x, dir.y, dir.z).normalize();
  ...
  static drawSphere(geo, pos, radius, color) {
    ...
    for (y = 0; y < h; y++) {
      ...
      for (x = 0; x < w; x++) {
        ...
        const v1 = verticesRows[y][x + 1] + start;
        const v1offset = v1 * 3;
        const v2 = verticesRows[y][x] + start;
        const v2offset = v2 * 3;
        const v3 = verticesRows[y + 1][x] + start;
        const v3offset = v3 * 3;
        const v4 = verticesRows[y + 1][x + 1] + start;
        const v4offset = v4 * 3;
