...
// in an attempt to reduce memory overhead, cache all Colors
...
export const CC = {
  ...
  color: function color_(hex) {
    ...
    // pass through Color & other objects
    return hex;
  },

  getHex(hex) {
    // eslint-disable-next-line radix
    if (!Number.isNaN(Number.parseInt(hex))) return Number.parseInt(hex);
...
ssColors.pyMol = {h: 0xff0000, s: 0xffff00, c: 0x00ff00};
ssColors.Jmol = {h: 0xff0080, s: 0xffc800, c: 0xffffff};
 ...
 * @type {Record<string, Record<string, number>> & {defaultColor: number}}
 */
export const elementColors = {};

elementColors.defaultColor = 0xff1493;
...
elementColors.defaultColors = elementColors.rasmol;

elementColors.greenCarbon = extend({}, elementColors.defaultColors);
elementColors.greenCarbon.C = 0x00ff00; // bright green

elementColors.cyanCarbon = extend({}, elementColors.defaultColors);
elementColors.cyanCarbon.C = 0x00ffff;

elementColors.magentaCarbon = extend({}, elementColors.defaultColors);
elementColors.magentaCarbon.C = 0xff00ff;

elementColors.yellowCarbon = extend({}, elementColors.defaultColors);
elementColors.yellowCarbon.C = 0xffff00;

elementColors.whiteCarbon = extend({}, elementColors.defaultColors);
elementColors.whiteCarbon.C = 0xffffff;

elementColors.orangeCarbon = extend({}, elementColors.defaultColors);
elementColors.orangeCarbon.C = 0xffa500;

elementColors.purpleCarbon = extend({}, elementColors.defaultColors);
elementColors.purpleCarbon.C = 0x800080;

elementColors.blueCarbon = extend({}, elementColors.defaultColors);
elementColors.blueCarbon.C = 0x0000ff;
...
export function getColorFromStyle(atom, style) {
  ...
  } else if (typeof schemeName == 'string' && schemeName.endsWith('Carbon')) {
    // any color you want of carbon
    const ccolor = schemeName.substring(0, schemeName.lastIndexOf('Carbon')).toLowerCase();
    if (typeof htmlColors[ccolor] != 'undefined') {
      const newscheme = extend({}, elementColors.defaultColors);
  ...
  let {color} = atom;
  if (typeof style.color != 'undefined' && style.color !== 'spectrum') color = style.color;
  if (typeof scheme != 'undefined') {
    ...
    if (typeof elementColors[scheme] != 'undefined') {
      ...
      if (scheme && typeof scheme[atom[scheme.prop]] != 'undefined') {
  ...
  } else if (style.colorfunc && typeof style.colorfunc != 'undefined') {
