...
// @ts-ignore
import $ from 'jquery';
import {builtinColorSchemes} from '../colors';
import {defaultCSS} from './defaultValues';
...
const Form = (function () {
  ...
  Form.ListInput = function (control, listElements) {
    ...
    select.css(defaultCSS.ListInput.select);
    ...
    this.validate = function () {
      if (control.value === 'select' || control.value == null) {
        if (this.showAlertBox) failMessage.show();
        select.css({
  ...
  Form.Input = function (control) {
    ...
    function checkInputFloat() {
      ...
      if (Number.isNaN(Number.parseFloat(inputString))) {
        return false;
      }
      return true;
    }
    ...
    function checkInputNumber() {
      ...
      if (Number.isNaN(Number.parseInt(inputString))) {
        return false;
      }
      return true;
    }
    ...
    function checkList(inputString, submit = false) {
      ...
      // eslint-disable-next-line radix
      if (Number.isNaN(Number.parseInt(rangeList[0]))) return false;
    ...
    function checkRangeInput(inputString, submit = false) {
      ...
      if (rangeInputs.length === 1) {
        if (Number.isNaN(Number.parseInt(rangeInputs[0]))) return false;
        return true;
      }
      if (rangeInputs.length === 2) {
        if (
          Number.isNaN(Number.parseInt(rangeInputs[0])) ||
          Number.isNaN(Number.parseInt(rangeInputs[1]))
        )
          return false;
        return true;
      }
    ...
    this.validateOnlyNumber = function (floatType = false) {
      ...
      input.on('keydown keyup paste cut', checkInput);
    };
    ...
    input.css(defaultCSS.Input.input);
    boundingBox.css(defaultCSS.Input.boundingBox);
  };

  /**
   * Create Checkbox input for boolean values
   * @function UI#Form.Checkbox
   * @param {Object} control Reference object to store the value
   */
  Form.Checkbox = function (control) {
    const label = $('<div></div>');
    label.text(control.key);
    label.css(defaultCSS.TextDefault);
   ...
   *
   * @function UI#Form
   * @param {any} specs the defination of spec is used as an input to generate the form
  ...
  function Form(specs, mainControl) {
    ...
    this.validate = function () {
      ...
      return validations.find(e => e === false) === undefined;
    };
    ...
    this.setValue = function (val) {
      ...
      for (let i = 0; i < keys.length; i++) {
        const input = inputs.find(e => {
          if (e.control.key === keys[i]) return e;
        });
      ...
      // @ts-ignore because this function does not take an argument in its abstract definition
    ...
    function Property(key, type) {
      ...
      if (specs[key].type === 'string' || specs[key].type === 'element') {
        ...
        if (
          specs[key].min !== undefined &&
          specs[key].max !== undefined &&
          specs[key].default !== undefined
        ) {
          slider = true;
        }
        ...
        this.placeholder = new Form.ListInput(control, Object.keys(builtinColorSchemes));
        this.placeholder.ui.attr('type', 'text');
        ...
        this.placeholder.ui.append($('<div></div>').css(defaultCSS.LinkBreak));
      } else {
      ...
      this.getValue = function () {
        if (this.placeholder.getValue) return this.placeholder.getValue();
        return null;
      };
      ...
      // @ts-ignore because this function does not exist on the placeholder { ui: $('<div></div>'), update: () => {}, getValue: () => control }
      if (this.placeholder.onUpdate) this.placeholder.onUpdate(updateValues);
    }
  }
