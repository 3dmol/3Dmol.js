...
import { isNumeric } from "./3dmol";
import { CC, getColorFromStyle } from "./colors";
import { GLDraw } from "./gldraw";
import { Gradient } from "./gradients";
import { FaceColors, Geometry, Line, LineBasicMaterial, LineStrip, Mesh, MeshDoubleLambertMaterial, Sphere, Triangle, Vector3 } from "./WebGL";
 ...
 *       follows:
 * @example export const download("pdb:4ZD3",viewer,{},function(){
...
export const subdivide_spline = function (_points, DIV) {
  // points as Vector3
  var ret = [];
  var points = _points;
...
const drawShapeStrip = function (
  ...
  for (j = 0; j < num; j++) {
    cs_ellipse.push(
      0.25 + (1.5 * Math.sqrt((num - 1) * j - Math.pow(j, 2))) / (num - 1)
    );
  ...
  for (i = 0; i < len; i++) {
    ...
    if (points[0][i].atom !== undefined) {
      ...
      if (shape === "oval") cs_shape = cs_ellipse;
      else if (shape === "rectangle") cs_shape = cs_rectangle;
      else if (shape === "parabola") cs_shape = cs_parabola;
    }
    ...
    for (j = 0; j < num; j++) {
      ...
      axis[j] = toSide
        .cross(toNext)
        .normalize()
        .multiplyScalar(thickness * cs_shape[j]);
    }
    ...
    for (j = 0; j < num; j++) cs_top[j] = points[j][i].clone().add(axis[j]);

    /*
     * Until this point the vertices have been dealt with as
     * export const Vector3() objects, but we need to serialize them into the
    ...
    if (i > 0) {
      ...
      if (currentAtom.clickable || currentAtom.hoverable) {
        ...
        faces.push(
          new Triangle(last_cs_bottom[0], cs_bottom[0], cs_bottom[num - 1])
        );
        faces.push(
          new Triangle(
            last_cs_bottom[0],
            cs_bottom[num - 1],
            last_cs_bottom[num - 1]
          )
        );
        ...
        faces.push(
          new Triangle(cs_top[0], last_cs_top[0], last_cs_top[num - 1])
        );
        faces.push(
          new Triangle(cs_top[num - 1], cs_top[0], last_cs_top[num - 1])
        );

        faces.push(
          new Triangle(cs_bottom[0], last_cs_bottom[0], last_cs_top[0])
        );
        faces.push(new Triangle(cs_top[0], cs_bottom[0], last_cs_top[0]));
...
const drawPlainStrip = function (geo, points, colors, div, thickness, opacity) {
  ...
  if (!thickness) return drawThinStrip(geo, p1, p2, colors, div, opacity);
...
const drawSmoothCurve = function (group, _points, width, colors, div) {
  ...
  var line = new Line(geo, lineMaterial);
  line.type = LineStrip;
  group.add(line);
};

const drawStrip = function (geo, points, colors, div, thickness, opacity, shape) {
...
const setGeo = function (group, geo, opacity, outline, setNormals) {
  ...
  var cartoonMesh = new Mesh(geo, cartoonMaterial);
  group.add(cartoonMesh);
};

const addBackbonePoints = function (
  points,
  num,
  smoothen,
  backbonePt,
  ...
  if (!backbonePt || !orientPt || !backboneAtom) return;
...
const drawCartoonBase = function (
  group,
  atomList,
  gradientrange,
  fill,
  ...
  for (var g in Gradient.builtinGradients) {
    if (Gradient.builtinGradients.hasOwnProperty(g)) {
      ...
      gradients[g] = new Gradient.builtinGradients[g](
        gradientrange[1],
        gradientrange[0]
      );
    }
  ...
  for (i in atomList) {
    ...
    if (next.elem === "C" && next.atom === "CA") {
      ...
      if (
        connected &&
        (curr.ss === "h" || curr.ss == "tube start") &&
        curr.style.cartoon.tubes
      ) {
  ...
  for (var a = 0; a < atoms.length; a++) {
    ...
    if (cartoon.style === "trace") {
      ...
      if (next.hetflag);
        ...
        if (isNumeric(cartoon.thickness)) thickness = cartoon.thickness;
        else thickness = defaultThickness;

        if (inConnectedResidues(curr, next)) {
          // if both atoms are same color, draw single cylinder
          if (nextColor == currColor) {
            var color = CC.color(nextColor);
            GLDraw.drawCylinder(shapeGeo, curr, next, thickness, color, 2, 2);
          } // otherwise draw cylinders for each color
          ...
          // export const Vector3 in this case
        }

        if (
          (next.clickable === true || next.hoverable) &&
          next.intersectionShape !== undefined
        ) {
    ...
    } // draw default-style cartoons based on secondary structure
    else {
      ...
      if (
        ...
        if (drawingTube) {
          if (next.ss === "tube end") {
            ...
            //don't span chains no matter what, check for short tubes (less than ideal)
            drawingTube = false;
            curr.ss = "h";
          ...
          } else continue; // don't accumulate strand points while
        ...
        if (
          curr &&
          (!inConnectedResidues(curr, next) || curr.ss === "tube start")
        ) {
          if (curr.ss === "tube start") {
            drawingTube = true;
            tubeStart = new Vector3(curr.x, curr.y, curr.z);
            curr.ss = "h";
          }

          if (baseEndPt) {
            // draw the last base if it's a NA chain
            if (terminalPt)
              baseStartPt = new Vector3()
                .addVectors(curr, terminalPt)
                .multiplyScalar(0.5);
            else baseStartPt = new Vector3(curr.x, curr.y, curr.z);
        ...
        if (
          curr === undefined ||
          curr.rescode != next.rescode ||
          curr.resi != next.resi
        ) {
          if (baseEndPt) {
            // draw last NA residue's base
            // start the cylinder at the midpoint between
            // consecutive backbone atoms
            baseStartPt = new Vector3()
              .addVectors(curr, next)
              .multiplyScalar(0.5);
            var startFix = baseStartPt
              .clone()
              .sub(baseEndPt)
        ...
        // click handling
        if (
          (next.clickable === true || next.hoverable === true) &&
          (next.intersectionShape === undefined ||
            next.intersectionShape.triangle === undefined)
        )
      ...
      // atoms used to orient the backbone strand
        ...
        (isAlphaCarbon(curr) && next.atom === "O") ||
        (inNucleicAcid &&
          curr.atom === "P" &&
          (next.atom === "OP2" || next.atom === "O2P")) ||
        (inNucleicAcid &&
          curr.atom.indexOf("O5") == 0 &&
        ...
        if (next.atom === "OP2" || next.atom === "O2P")
          // for NA 3'
          // terminus
          termOrientPt = new Vector3(next.x, next.y, next.z);
      }
      ...
      // purines and pyramidines)
        ...
        (next.atom === "N1" && nextresn in purResns) ||
        (next.atom === "N3" && nextresn in pyrResns)
      ) {
        baseEndPt = new Vector3(next.x, next.y, next.z);
        baseEndPt.color = getColorFromStyle(next, cartoon).getHex();
      }
...
export const drawCartoon = function (group, atomList, gradientrange, quality) {
  quality = quality || 5;
  drawCartoonBase(
    group,
    atomList,
