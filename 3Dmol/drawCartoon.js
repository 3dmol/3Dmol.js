...
function drawCartoonBase(group, atomList, gradientrange, fill, doNotSmoothen, num, div) {
  ...
  for (a = 0; a < atoms.length; a++) {
    ...
    if (
      curr &&
      curr.style.cartoon &&
      (!next.style.cartoon || curr.style.cartoon.opacity !== next.style.cartoon.opacity)
    ) {
    ...
    if (cartoon.style === 'trace') {
       ...
       */
      if (
        !next.hetflag /** ignore non-protein atoms */ &&
        ((next.elem === 'C' && next.atom === 'CA') || (inNucleicAcid && next.atom === 'P'))
      ) {
        ...
        if (inConnectedResidues(curr, next)) {
          ...
          // (split down the middle)
            ...
            const midpoint = new Vector3().addVectors(curr, next).multiplyScalar(0.5);
            const color1 = CC.color(currColor);
            const color2 = CC.color(nextColor);
            GLDraw.drawCylinder(shapeGeo, curr, midpoint, thickness, color1, 2, 0);
            GLDraw.drawCylinder(shapeGeo, midpoint, next, thickness, color2, 0, 2);
      ...
      if (
        ...
        if (drawingTube) {
          if (next.ss === 'tube end') {
            ...
            GLDraw.drawCylinder(shapeGeo, tubeStart, tubeEnd, 2, CC.color(currColor), 1, 1);
            next.ss = 'h';
        ...
        if (curr && (!inConnectedResidues(curr, next) || curr.ss === 'tube start')) {
          ...
          if (baseEndPt) {
            ...
            if (terminalPt)
              baseStartPt = new Vector3().addVectors(curr, terminalPt).multiplyScalar(0.5);
        ...
        if (curr === undefined || curr.rescode !== next.rescode || curr.resi !== next.resi) {
          if (baseEndPt) {
            ...
            // consecutive backbone atoms
            baseStartPt = new Vector3().addVectors(curr, next).multiplyScalar(0.5);
            const startFix = baseStartPt.clone().sub(baseEndPt).multiplyScalar(0.02); // TODO: apply this
            // as function of
            // thickness
  ...
  if (baseEndPt) {
    ...
    if (terminalPt) baseStartPt = new Vector3().addVectors(curr, terminalPt).multiplyScalar(0.5);
    else baseStartPt = new Vector3(curr.x, curr.y, curr.z);

    GLDraw.drawCylinder(shapeGeo, baseStartPt, baseEndPt, 0.4, CC.color(baseEndPt.color), 0, 2);
    addBackbonePoints(
      points,
      num,
      !doNotSmoothen,
...
export default function drawCartoon(group, atomList, gradientrange, quality) {
  quality = quality || 5;
  drawCartoonBase(group, atomList, gradientrange, true, false, quality, quality);
}
