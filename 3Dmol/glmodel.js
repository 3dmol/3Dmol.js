...
import netcdfjs from "netcdfjs/dist/netcdfjs";
import pako from "pako";
import { deepCopy, getAtomProperty, getbin, getExtent, getPropertyRange, makeFunction, specStringToObject } from "./3dmol";
import { CC, elementColors, getColorFromStyle } from "./colors";
import { drawCartoon } from "./glcartoon";
import { GLDraw } from "./gldraw";
import { Gradient } from "./gradients";
import { GLViewer } from "./glviewer";
import { Color, conversionMatrix3, Cylinder, Geometry, InstancedMaterial, Line, LineBasicMaterial, LinePieces, Matrix3, Matrix4, Mesh, MeshLambertMaterial, Object3D, Sphere, SphereImposterMaterial, StickImposterMaterial, Vector3 } from "./WebGL";
 ...
 * @constructor
 * @param {number=} mid
 * @param {Object=} options Object defining default atom colors as atom => color property value pairs
 * @see download
 */
export const GLModel = function (mid, options) {
  

  
  // class functions
  // return true if a and b represent the same style
  this.sameObj = function (a, b) {
  ...
  var defaultColor = elementColors.defaultColor;

  options = options ? options : {};
  var ElementColors = options.defaultcolors
  ...
  // sphere drawing
  var defaultSphereRadius = options.defaultSphereRadius
    ? options.defaultSphereRadius
    : 1.5;

  var defaultCartoonQuality = options.cartoonQuality
   ...
   * @prop {import("./colors").ColorschemeSpec} colorscheme - element based coloring
   * @prop {import("./colors").ColorSpec} color - fixed coloring, overrides colorscheme
  ...
  var drawAtomSphere = function (atom, geo) {
    ...
    if (
      (atom.clickable === true || atom.hoverable) &&
      atom.intersectionShape !== undefined
    ) {
  ...
  var createMolObj = function (atoms, options) {
    ...
    for (i = 0, n = atoms.length; i < n; i++) {
      ...
      if (atom && atom.style) {
        ...
        if (
          typeof atom.style.cartoon !== "undefined" &&
    ...
    if (cartoonAtoms.length > 0) {
      drawCartoon(ret, cartoonAtoms, range, defaultCartoonQuality);
    }
    ...
    if (stickGeometry.vertices > 0) {
      ...
      if (
        !balls ||
        typeof balls.vertices === "undefined" ||
        balls.vertices == 0
      )
        balls = null; //no balls
  ...
  this.getInternalState = function () {
    return { atoms: atoms, frames: frames };
  };
  ...
  this.getCrystData = function () {
    if (modelData.cryst) {
      ...
      if (!modelData.cryst.matrix) {
        ...
        modelData.cryst.matrix = conversionMatrix3(
          cryst.a,
          cryst.b,
          cryst.c,
          cryst.alpha,
  ...
  this.setFrame = function (framenum, viewer) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (frames.url != undefined) {
        var url = frames.url;
        getbin(
            url + "/traj/frame/" + framenum + "/" + frames.path,
            null,
            "POST"
          )
  ...
  this.vibrate = function (numFrames, amplitude, bothWays, viewer, arrowSpec) {
    ...
    for (var i = start; i < end; i++) {
      ...
      for (var j = 0; j < atoms.length; j++) {
        var dx = getAtomProperty(atoms[j], "dx");
        var dy = getAtomProperty(atoms[j], "dy");
        var dz = getAtomProperty(atoms[j], "dz");
        var newVector = new Vector3(dx, dy, dz);
        var starting = new Vector3(atoms[j].x, atoms[j].y, atoms[j].z);
        ...
        if (viewer && arrowSpec) {
          var spec = Object.assign({}, arrowSpec);
  ...
  this.addMolData = function (data, format, options) {
    options = options || {};
    var parsedAtoms = GLModel.prototype.parseMolData(
      data,
      format,
      options
    );
    ...
    if (
      options.vibrate &&
      options.vibrate.frames &&
      options.vibrate.amplitude
    ) {
  ...
  var expandAtomList = function (atomList, amt) {
    ...
    var pb = getExtent(atomList); // previous bounding box
  ...
  this.setStyle = function (sel, style, add) {
    ...
    var s;
    for (s in sel) {
      if (!GLModel.prototype.validAtomSelectionSpecs.hasOwnProperty(s)) {
        console.log("Unknown selector " + s);
      }
    }
    // report to console if this is not a valid style
    for (s in style) {
      if (!GLModel.prototype.validAtomStyleSpecs.hasOwnProperty(s)) {
  ...
  this.setColorByElement = function (sel, colors) {
    if (molObj !== null && this.sameObj(colors, lastColors)) return; // don't recompute
    lastColors = colors;
  ...
  /**
   * @function GLModel.prototype.setColorByProperty
   * @param {type} sel
   * @param {type} prop
   * @param {type} gradient
   */
  ...
  this.removegl = function (group) {
    if (renderedMolObj) {
      ...
      if (renderedMolObj.geometry !== undefined)
        renderedMolObj.geometry.dispose();
      if (renderedMolObj.material !== undefined)
        renderedMolObj.material.dispose();
      group.remove(renderedMolObj);
      renderedMolObj = null;
    }
    molObj = null;
...
*/
  ...
  this.setCoordinates = function (str, format) {
    ...
    if (supportedFormats.hasOwnProperty(format)) {
      ...
      var values = GLModel.prototype.parseCrd(str, format);
      var count = 0;
  ...
  this.addAtomSpecs = function (customAtomSpecs) {
    for (var i = 0; i < customAtomSpecs.length; i++) {
      if (
        !GLModel.prototype.validAtomSelectionSpecs.hasOwnProperty(
          customAtomSpecs[i]
        )
      ) {
        GLModel.prototype.validAtomSelectionSpecs[customAtomSpecs[i]] =
          {};
      }
    }
  };
...
GLModel.prototype.validElements = [
  "H",
  "Li",
  "LI",
  "Na",
...
GLModel.prototype.validAtomSpecs = {
...
GLModel.prototype.validAtomSelectionSpecs = Object.assign(
  {},
  GLModel.prototype.validAtomSpecs,
  validExtras
);
...
GLModel.prototype.validAtomStyleSpecs = {
...
GLModel.prototype.validSurfaceSpecs = {
  opacity: {
    type: "number",
    floatType: true,
    gui: true,
...
GLModel.prototype.validLabelResSpecs = {
...
GLModel.prototype.parseCrd = function (data, format) {
...
GLModel.prototype.parseMolData = function (data, format, options) {
  format = format || "";
