...
$3Dmol.GLModel = (function () {
  ...
  function GLModel(mid, options) {
    ...
    options = options || {};
    const ElementColors = options.defaultcolors
    ...
    const drawAtomCross = function (atom, geos) {
      ...
      if (!geos[linewidth]) geos[linewidth] = new $3Dmol.Geometry();
        ...
        if (atom.index < atom2.index) {
          ...
          if (atom.bondOrder[i] === 1 || singleBond || atom.bondOrder[i] > 3) {
            ...
            if (atom.bondOrder[i] == 2) {
              ...
              if (C1 != C2) {
                mp = new $3Dmol.Vector3().addVectors(p1a, p2a).multiplyScalar(0.5);
                mp2 = new $3Dmol.Vector3().addVectors(p1b, p2b).multiplyScalar(0.5);
              ...
              if (atomneedsi || atom2needsi) {
                if (!mp) mp = new $3Dmol.Vector3().addVectors(p1a, p2a).multiplyScalar(0.5);
                if (!mp2) mp2 = new $3Dmol.Vector3().addVectors(p1b, p2b).multiplyScalar(0.5);
                if (atomneedsi) {
              ...
              if (C1 != C2) {
                mp = new $3Dmol.Vector3().addVectors(p1a, p2a).multiplyScalar(0.5);
                mp2 = new $3Dmol.Vector3().addVectors(p1b, p2b).multiplyScalar(0.5);
                mp3 = new $3Dmol.Vector3().addVectors(p1, p2).multiplyScalar(0.5);
                drawCyl(geo, p1a, mp, r, C1, mfromCap, 0);
                drawCyl(geo, mp, p2a, r, C2, 0, mtoCap);
              ...
              if (atomneedsi || atom2needsi) {
                if (!mp) mp = new $3Dmol.Vector3().addVectors(p1a, p2a).multiplyScalar(0.5);
                if (!mp2) mp2 = new $3Dmol.Vector3().addVectors(p1b, p2b).multiplyScalar(0.5);
                if (!mp3) mp3 = new $3Dmol.Vector3().addVectors(p1, p2).multiplyScalar(0.5);
    ...
    const createMolObj = function (atoms, options) {
      ...
      for (i in lineGeometries) {
        if (lineGeometries.hasOwnProperty(i)) {
          ...
          const line = new $3Dmol.Line(lineGeometries[i], lineMaterial, $3Dmol.LinePieces);
      ...
      for (i in crossGeometries) {
        if (crossGeometries.hasOwnProperty(i)) {
          ...
          const cross = new $3Dmol.Line(crossGeometries[i], crossMaterial, $3Dmol.LinePieces);
    ...
    this.setCrystMatrix = function (matrix) {
      matrix = matrix || new $3Dmol.Matrix3(1, 0, 0, 0, 1, 0, 0, 0, 1);
     ...
     * @return {Promise}
     */
    this.setFrame = function (framenum, viewer) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (frames.url != undefined) {
          const {url} = frames;
          $3Dmol
            .getbin(`${url}/traj/frame/${framenum}/${frames.path}`, null, 'POST')
            .then(buffer => {
    ...
    this.addMolData = function (data, format, options) {
      ...
      if (frames.length == 0) {
        ...
        for (let i = 0; i < parsedAtoms.length; i++) {
          if (parsedAtoms[i].length != 0) frames.push(parsedAtoms[i]);
        }
        if (frames[0]) atoms = frames[0];
      } else {
        // subsequent calls
        if (options.frames) {
          // add to new frame
    ...
    this.selectedAtoms = function (sel, from) {
      ...
      if (
        sel.hasOwnProperty('within') &&
        sel.within.hasOwnProperty('sel') &&
    ...
    this.addAtoms = function (newatoms) {
      ...
      for (i = 0; i < newatoms.length; i++) {
        ...
        if (typeof a.color == 'undefined') a.color = ElementColors[a.elem] || defaultColor;
    ...
    this.setStyle = function (sel, style, add) {
      ...
      if (changedAtoms) molObj = null; // force rebuild
    };
    ...
    this.setColorByElement = function (sel, colors) {
      ...
      if (molObj !== null && sameObj(colors, lastColors)) return; // don't recompute
      lastColors = colors;
