 ...
 */
export default function parseVASP(str, options) {
  ...
  lattice.xVec = new Float32Array(lines[2].replace(/^\s+/, '').split(/\s+/).map(Number.parseFloat));
  lattice.yVec = new Float32Array(lines[3].replace(/^\s+/, '').split(/\s+/).map(Number.parseFloat));
  lattice.zVec = new Float32Array(lines[4].replace(/^\s+/, '').split(/\s+/).map(Number.parseFloat));
  ...
  const atomSpeciesNumber = new Int16Array(lines[6].replace(/^\s+/, '').split(/\s+/).map(Number.parseFloat));
  ...
  for (let i = 0, len = atomSymbols.length; i < len; i++) {
    ...
    for (let j = 0, atomLen = atomSpeciesNumber[i]; j < atomLen; j++) {
      const coords = new Float32Array(lines[atomCounter + j].replace(/^\s+/, '').split(/\s+/).map(Number.parseFloat));
      ...
      if (vaspMode === 'cartesian') {
        ...
        atom.x =
          lattice.length *
          (coords[0] * lattice.xVec[0] + coords[1] * lattice.yVec[0] + coords[2] * lattice.zVec[0]);
        atom.y =
          lattice.length *
          (coords[0] * lattice.xVec[1] + coords[1] * lattice.yVec[1] + coords[2] * lattice.zVec[1]);
        atom.z =
          lattice.length *
          (coords[0] * lattice.xVec[2] + coords[1] * lattice.yVec[2] + coords[2] * lattice.zVec[2]);
      }

      atom.bonds = [];

      // @ts-ignore atom is not valid because it does not have an index property
      atoms[0].push(atom);
    }
