...
import { isEmptyObject, extend } from "jquery";
 ...
 */
export const GLViewer = function (element, config) {
  // set variables
  config = config || {};
  var callback = config.callback;
  var defaultcolors = config.defaultcolors;
  if (!defaultcolors) defaultcolors = elementColors.defaultColors;
  ...
  if (typeof config.backgroundColor != undefined) {
    bgColor = CC.color(config.backgroundColor).getHex();
  }
  config.backgroundAlpha =
    config.backgroundAlpha == undefined ? 1.0 : config.backgroundAlpha;
  ...
  var camera = new Camera(fov, ASPECT, NEAR, FAR, config.orthographic);
  camera.position = new Vector3(camerax, 0, CAMERA_Z);
  var lookingAt = new Vector3();
  camera.lookAt(lookingAt);

  var raycaster = new Raycaster(new Vector3(0, 0, 0), new Vector3(0, 0, 0));
  ...
  // UI variables
  var cq = new Quaternion(0, 0, 0, 1);
  var dq = new Quaternion(0, 0, 0, 1);
  var animated = 0;
  ...
  var initializeScene = function () {
    scene = new Scene();
    scene.fog = new Fog(bgColor, 100, 200);

    modelGroup = new Object3D();
    rotationGroup = new Object3D();
    rotationGroup.useQuaternion = true;
    rotationGroup.quaternion = new Quaternion(0, 0, 0, 1);
  ...
  scene.fog.color = CC.color(bgColor);
   ...
   * @param {number} x position in screen coordinates
   * @param {number} y position in screen coordinates
   * @param {Array<any>} objects list of objects or selection object specifying what object to check for targeting
   */
   ...
   * @param {{x:number;y:number;z:number}|Array<{x:number;y:number;z:number}>} coords an object or list of objects with x,y,z attributes (e.g. an atom)
   * @return {{x:number;y:number;}|Array<{x:number;y:number;}>} - and object or list of {x: screenX, y: screenY}
   */
  ...
  var setHover = function (selected, event) {
    ...
    if (current_hover) {
      if (typeof current_hover.unhover_callback != "function") {
        current_hover.unhover_callback = makeFunction(
          current_hover.unhover_callback
        );
      }
  ...
  var handleHoverContinue = function (mouseX, mouseY) {
    ...
    if (
      ...
      intersects[0].clickable !== current_hover
    ) {
      setHover(null);
    }
  };

  var calcTouchDistance = function (ev) {
    // distance between first two
    // fingers
    var xdiff =
      ev.originalEvent.targetTouches[0].pageX -
      ev.originalEvent.targetTouches[1].pageX;
    var ydiff =
      ev.originalEvent.targetTouches[0].pageY -
   ...
   * @param {number} modelz coordinate in model coordinates to compute offset for, default is model axis
   * @function GLViewer#screenOffsetToModel
   */
  var screenOffsetToModel = (this.screenOffsetToModel = function (
    x,
    y,
    modelz
  ) {
   ...
   * @param {{x:number;y:number}} screen screen coordinate
   * @param {unknown} screen model coordinate
   * @function GLViewer#screenToModelDistance
   */
  ...
  /**
         ...
         * @param {Record<string, any>} parameters new camera parameters, with possible fields
  ...
  var _handleMouseScroll = (this._handleMouseScroll = function (ev) {
    // Zoom
    ev.preventDefault();
    if (!scene) return;
    ...
    if (ev.originalEvent.detail) {
      rotationGroup.position.z +=
        (mult * scaleFactor * ev.originalEvent.detail) / 10;
    } else if (ev.originalEvent.wheelDelta) {
      rotationGroup.position.z -=
        (mult * scaleFactor * ev.originalEvent.wheelDelta) / 400;
    }
    rotationGroup.position.z = adjustZoomToLimits(rotationGroup.position.z);
    show();
  ...
  _stateManager = new StateManager(_viewer, config); // Creates the UI state management tool
  /**
   * Set the duration of the hover delay
   *
   * @function GLViewer#setHoverDuration
   * @param {number} [duration] - an optional parameter that denotes
  ...
  var hoverTimeout;
  var _handleMouseMove = (this._handleMouseMove = function (ev) {
    // touchmove
    ...
    if (mode == 3 || (mouseButton == 3 && ev.ctrlKey)) {
      ...
      // Translate
      var t = screenOffsetToModel(
        ratioX * (x - mouseStartX),
        ratioY * (y - mouseStartY)
      );
  ...
  /**
         ...
         *
         * @function GLViewer#setProjection
         *
         * @example
  ...
  /**
         ...
         * @param {number | GLModel} [id] - Retrieve model with specified id, or first model if id is not specified
  ...
  this.getModel = function (id) {
    ...
    if (!(id in models)) {
      ...
      else return models[models.length - 1]; //get last model if no (or invalid) id specified
    }
    return models[id];
  };
  ...
  /**
         ...
         * @param {string} [axis] - Axis ("x", "y", "z", "vx", "vy", or "vz") to rotate around.
         *            Default "y".  View relative (rather than model relative) axes are prefixed with v.
  ...
  this.rotate = function (angle, axis, animationDuration, fixedPath) {
    ...
    if (animationDuration) {
      var final = new Quaternion().copy(rotationGroup.quaternion).multiply(q); //final
      animateMotion(
        animationDuration,
        fixedPath,
        modelGroup.position,
  ...
  this.render = function (callback, exts) {
    ...
    for (i = 0; i < labels.length; i++) {
      if (
        labels[i] &&
        typeof labels[i].frame != "undefined" &&
        labels[i].frame >= 0
      ) {
  ...
  this.translate = function (x, y, animationDuration, fixedPath) {
    ...
    var dx = x / WIDTH;
    var dy = y / HEIGHT;
    var v = new Vector3(0, 0, -CAMERA_Z);
  ...
  this.zoomTo = function (sel, animationDuration, fixedPath) {
    ...
    var finalpos = center.clone().multiplyScalar(-1);
    var finalz = -(
      (maxD * 0.5) / Math.tan(((Math.PI / 180.0) * camera.fov) / 2) -
      CAMERA_Z
    );
  ...
  this.addLabel = function (text, options, sel, noshow) {
    options = options || {};
    if (sel) {
      var extent = getExtent(getAtomsFromSel(sel));
  ...
  function addLineDashed(spec, s) {
    ...
    var p2 = new Vector3(spec.end.x, spec.end.y || 0, spec.end.z || 0);
  ...
  this.animate = function (options) {
    ...
    var display = function (direction) {
      ...
      if (direction == "forward") {
        that.setFrame(currFrame).then(function () {
          currFrame = (currFrame + inc) % mostFrames;
          resolve();
        });
      } else if (direction == "backward") {
        that.setFrame(mostFrames - 1 - currFrame).then(function () {
          currFrame = (currFrame + inc) % mostFrames;
          resolve();
        });
      } else {
  ...
  this.addModel = function (data, format, options) {
    if (options && !options.defaultcolors) {
      ...
      options = {
        ...
        cartoonQuality: config.cartoonQuality,
      };
    }
    var m = new GLModel(models.length, options);
  ...
  this.addModels = function (data, format, options) {
    ...
    for (var i = 0; i < modelatoms.length; i++) {
      var newModel = new GLModel(models.length, defaultcolors);
  ...
  this.addModelsAsFrames = function (data, format, options) {
    ...
    options.multimodel = true;
    options.frames = true;
    var m = new GLModel(models.length, defaultcolors);
    m.addMolData(data, format, options);
    models.push(m);
   ...
   * @return {Object}
   */
  var generateMeshSyncHelper = function (
    type,
    expandedExtent,
    extendedAtoms,
    atomsToShow,
    ...
    var addSurfaceHelper = function addSurfaceHelper(
      ...
      if (sync) {
        ...
        var callSyncHelper = function callSyncHelper(i) {
          return new Promise(function (resolve) {
            var VandF = generateMeshSyncHelper(
              type,
              extents[i].extent,
              extents[i].atoms,
              extents[i].toshow,
        ...
        for (let i = 0, il = numWorkers; i < il; i++) {
          var w = new Worker(SurfaceWorker);
          workers.push(w);
          w.postMessage({
            type: -1,
            atoms: reducedAtoms,
  ...
  this.setAutoEyeSeparation = function (isright, x) {
    ...
    camera.lookAt(new Vector3(0, 0, rotationGroup.position.z));
