    ...
    dx(str) {
      ...
      for (i = 0; i < lines.length; i++) {
        ...
        if ((m = recounts.exec(line))) {
          const nX = Number.parseInt(m[1], 10) || Number.parseInt(m[1], 16);
          const nY = Number.parseInt(m[2], 10) || Number.parseInt(m[2], 16);
          const nZ = Number.parseInt(m[3], 10) || Number.parseInt(m[3], 16);
          this.size = {x: nX, y: nY, z: nZ};
