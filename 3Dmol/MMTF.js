...
function getView(ctor, typedArray, elemSize) {
  return typedArray
    ? new ctor(typedArray.buffer, typedArray.byteOffset, typedArray.byteLength / (elemSize || 1))
    : undefined;
}
...
function decodeInt32(bytes, output) {
  ...
  for (let i = 0, i4 = 0; i < n; ++i, i4 += 4) {
    output[i] =
      (bytes[i4] << 24) ^ (bytes[i4 + 1] << 16) ^ (bytes[i4 + 2] << 8) ^ (bytes[i4 + 3] << 0);
  }
  return output;
}
...
function decodeMmtf(inputDict, params) {
  params = params || {};
  const ignoreFields = params.ignoreFields;
  const outputDict = {};
...
function traverseMmtf(mmtfData, eventCallbacks, params) {
  ...
  const firstModelOnly = params.firstModelOnly;

  // setup callbacks
  const onModel = eventCallbacks.onModel;
  const onChain = eventCallbacks.onChain;
  const onGroup = eventCallbacks.onGroup;
  const onAtom = eventCallbacks.onAtom;
  const onBond = eventCallbacks.onBond;
  ...
  const chainNameList = mmtfData.chainNameList;
  const secStructList = mmtfData.secStructList;
  const insCodeList = mmtfData.insCodeList;
  const sequenceIndexList = mmtfData.sequenceIndexList;
  const atomIdList = mmtfData.atomIdList;
  ...
  for (o = 0, ol = mmtfData.chainsPerModel.length; o < ol; ++o) {
    ...
    for (i = 0; i < modelChainCount; ++i) {
      ...
      if (onChain) {
        ...
        let chainName = null;
        if (chainNameList) {
          chainName = fromCharCode(chainNameList.subarray(chainIndex * 4, chainIndex * 4 + 4));
        }
        onChain({
    ...
    if (onBond) {
      ...
      if (bondAtomList) {
        for (k = 0, kl = bondAtomList.length; k < kl; k += 2) {
          ...
          if (
            (atomIndex1 >= modelFirstAtomIndex && atomIndex1 <= modelLastAtomIndex) ||
